...
kruskal.test(log_ave_lit ~ bins,
             data = aq_ins_clean_bins,
             subset = taxon == "Ostracod") # Kruskal-Wallis chi-squared = 6.7235, df = 2, p-value = 0.03467
...
pairwise.wilcox.test(
  ...
  bins_ostracods$bins,
  p.adjust.method = "bonferroni") # p = 0.051 for comparison btwn moderate and high DO level bins
```
...
```{r}
#| label: comparing-abundance-proportions-across-DO-levels


# Made a copy of the composition plot but changed the bins to match the 4 quartiles of the distribution plot
...
copepods_matrix <- matrix(c(160.133333,1112.133333, 2155.200000,949.866667, 17.466667,165.066666), nrow = 3, ncol = 2, byrow = TRUE,
...
ostracods_matrix <- matrix(c(592.666667,679.599963, 211.466667,2893.600033, 153.733333,28.800037), nrow = 3, ncol = 2, byrow = TRUE,
                      dimnames = list(Oxygen = c("Low", "Moderate", "High"),
                                      Taxa = c("Copepod", "Other")))
...
cladocera_matrix <- matrix(c(509.333333,762.933333, 688.400000,2417.06667, 10.000000,155.066666), nrow = 3, ncol = 2, byrow = TRUE,
                      dimnames = list(Oxygen = c("Low", "Moderate", "High"),
                                      Taxa = c("Copepod", "Other")))
...
boatman_matrix <- matrix(c(10.133333,1262.133333, 50.000000,3055.466667, 1.333333,181.2), nrow = 3, ncol = 2, byrow = TRUE,
                      dimnames = list(Oxygen = c("Low", "Moderate", "High"),
                                      Taxa = c("Copepod", "Other")))
view(boatman_matrix)
...
When comparing medians of taxon abundance across DO bins, only Ostracod abundance is significantly different between the moderate and high DO bins. When comparing proportions of taxon abundnace across DO bins, the proportions of Copepods, Ostracods, and Cladocera significantly differ across the different DO bins. For Copepods, the differences exist between Low & High and Moderate & High DO levels. For Ostracods and Cladocera, the differences exist between all pairings of DO levels. 
